...
// Helper to validate CNPJ format (XX.XXX.XXX/XXXX-XX)
const cnpjRegex = /^\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}$/;
// Helper to validate CPF format (XXX.XXX.XXX-XX)
const cpfRegex = /^\d{3}\.\d{3}\.\d{3}-\d{2}$/;
// Helper to validate CEP format (XXXXX-XXX)
const cepRegex = /^\d{5}-\d{3}$/;
// Helper to validate Date format (DD/MM/YYYY)
const dateRegex = /^\d{2}\/\d{2}\/\d{4}$/;

export const contractDataSchema = z.object({
  'RAZAO SOCIAL DO CONTRATANTE': z.string().min(3, "Razão social muito curta"),
  'CNPJ DO CONTRATANTE': z.string().regex(cnpjRegex, "CNPJ inválido (use 00.000.000/0000-00)"),
  'CEP DO CONTRATANTE': z.string().regex(cepRegex, "CEP inválido (use 00000-000)"),
  'LOGRADOURO DO CONTRATANTE': z.string().min(3, "Logradouro obrigatório"),
  ...
  'CARGO DO REPRESENTANTE': z.string().min(2, "Cargo obrigatório"),
