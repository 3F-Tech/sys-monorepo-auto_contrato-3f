<template>
  <div class="min-h-screen bg-brand-deep text-white font-sans selection:bg-brand-cyan/30">
    ...
    <main class="max-w-7xl mx-auto px-6 py-10 space-y-8">
      ...
      <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div
          v-for="i in 6"
          :key="i"
          class="h-40 rounded-2xl bg-brand-offset/50 animate-pulse border border-brand-glass-border"
        ></div>
      </div>
        ...
        >
          ...
          <div class="relative z-10 space-y-4">
            <div class="flex items-start justify-between">
              <div class="flex items-center gap-4">
                <div
                  :style="{ borderColor: item.color as any }"
                  class="h-12 w-12 rounded-xl bg-brand-surface border flex items-center justify-center overflow-hidden"
                >
    ...
    <!-- Modals -->
    <BusinessModal :is-open="modalOpen" :business="selectedBusiness" @close="closeModal" @saved="loadBusiness" />
...
<script setup lang="ts">
  ...
  import { Building2, PlusCircle, Search, Edit3, Trash2, ArrowLeft } from '@lucide/vue'
  ...
  const filteredBusiness = computed(() => {
    ...
    return businessList.value.filter(
      (b) => b.name?.toLowerCase().includes(query) || b.cnpj?.toLowerCase().includes(query),
    )
  })
