    ...
    <main class="max-w-7xl mx-auto px-6 py-8 space-y-4">
      ...
      >
        ...
        <div v-if="user?.type === 'head'" class="flex p-1 bg-white/5 rounded-xl border border-white/5">
          <button
            @click="dashboardFilterType = 'team'"
            :class="
              dashboardFilterType === 'team' && selectedTeamId && !selectedTeamId?.startsWith('head_own_')
                ? 'bg-brand-cyan text-brand-deep shadow-lg scale-105'
                : 'text-white/40 hover:text-white'
            "
            class="px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all duration-300"
        ...
        <div
          v-if="['seller', 'sdr'].includes(user?.type || '') && user?.team_id"
          class="flex p-1 bg-white/5 rounded-xl border border-white/5"
        >
          <button
            @click="selectedSellerId = user?.id?.toString() || ''; selectedTeamId = ''"
            :class="
              selectedSellerId ? 'bg-brand-cyan text-brand-deep shadow-lg scale-105' : 'text-white/40 hover:text-white'
            "
            class="px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all duration-300"
          >
            Minha Meta
          ...
          <button
            ...
            :class="
              selectedTeamId ? 'bg-brand-cyan text-brand-deep shadow-lg scale-105' : 'text-white/40 hover:text-white'
            "
            class="px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all duration-300"
          >
            Meta Equipe
        ...
        <div
          v-if="dashboardFilterType === 'bu' && ['admin', 'coord', 'head'].includes(user?.type || '')"
        ...
        <template v-if="dashboardFilterType === 'team' || user?.type === 'seller'">
          ...
          <div
            v-if="
              ['admin', 'coord', 'head'].includes(user?.type || '') &&
              !(user?.type === 'head' && selectedTeamId?.startsWith('head_own_'))
            "
      ...
      <div class="space-y-8">
        ...
        <section>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
            <div
              v-for="(stat, index) in stats.filter((s) => s.type === 'finance')"
              :key="stat.label"
            ...
            >
              ...
              <div class="space-y-1 relative z-10">
                <div class="flex items-center gap-1.5">
                  <p class="text-white/50 text-[10px] font-bold uppercase tracking-[0.1em] leading-tight">
                    {{ stat.label }}
                  </p>
                  <Info
                    v-if="stat.tooltip"
      ...
      <section class="space-y-6">
        <ContractList
          ...
          :isHead="user?.type === 'head'"
          :isLeadership="['head', 'coord', 'admin'].includes(user?.type || '')"
          :filterMode="contractFilterMode"
          :businessUnits="businessList"
          :sellers="sellerStore.allSellers"
          :loading="isLoading"
      ...
      <section v-if="false" class="space-y-6">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center justify-between gap-4">
            ...
            <span class="text-[10px] font-bold text-white/20 uppercase tracking-widest">
              {{ visibleUsers.length }} Integrantes
            </span>
          </div>
        </div>
        ...
        <div v-if="sellerStore.loading" class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div
            v-for="i in 3"
            :key="i"
            class="h-32 rounded-2xl bg-brand-offset/50 animate-pulse border border-brand-glass-border"
          ></div>
        </div>

        <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div
            v-for="seller in visibleUsers"
            :key="seller.id"
          ...
          >
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <p class="text-[10px] text-white/40 uppercase tracking-wider font-medium">
                  {{ getRoleLabel(seller.type) }}
                </p>
              </div>
            </div>
          </div>
      ...
      <section
        v-if="['admin', 'head', 'coord'].some((r) => r === user?.type)"
      ...
      >
        ...
        <div class="space-y-2">
          ...
          <p class="text-white/40 text-sm max-w-md mx-auto">
            {{
              user?.type === 'admin'
                ? 'Gerencie usuários, permissões e unidades de negócio de toda a plataforma.'
        ...
        <div class="flex flex-wrap justify-center gap-4">
          ...
          <button
            ...
            @click="router.push('/admin/business')"
...
<script setup lang="ts">
  ...
  const dashboardFilterType = ref<'bu' | 'team' | 'coord'>(
    authStore.user?.type === 'head' || authStore.user?.type === 'admin' ? 'bu' : 'team',
  )
  ...
  const currentPerformance = computed(() => {
    ...
    const implementation = genContracts.reduce(
      (acc, curr) => acc + (parseFloat(curr.implementation_fee as any) || 0),
      0,
    )
  ...
  const activeGoal = computed(() => {
    ...
    if (dashboardFilterType.value === 'bu') {
      const buId =
        selectedBUId.value ||
        (user.value?.type === 'coord' ? (user.value as any)?.seller_business?.[0]?.business_id?.toString() : null)
      if (!buId) return null
      ...
      if (buId === '99' && (user.value?.type === 'head' || user.value?.type === 'coord')) {
        ...
        const buGoals = goalStore.goals.filter(
          (g) =>
            g.target_type === 'bu' && myBUIds.includes(g.target_id.toString()) && g.month === month && g.year === year,
        )
    ...
    if (dashboardFilterType.value === 'coord') {
      ...
      const teamHead = sellerStore.allSellers.find((s) => s.id?.toString() === selectedCoordId.value)
      const buIds = (teamHead as any)?.seller_business?.map((sb: any) => sb.business_id.toString()) || []
      if (buIds.length === 0) return null

      const buGoals = goalStore.goals.filter(
        (g) => g.target_type === 'bu' && buIds.includes(g.target_id.toString()) && g.month === month && g.year === year,
      )
      return sumGoals(buGoals, 'coord', selectedCoordId.value, month, year) || null
    }
    ...
    if (teamIdStr.startsWith('team_')) {
      ...
      const directTeamGoal = goalStore.goals.find(
        (g) =>
          g.target_type === 'team' &&
      ...
      const memberGoals = goalStore.goals.filter(
        (g) =>
    ...
    const members = sellerStore.allSellers.filter(
      (s) => s.head_id?.toString() === teamIdStr || s.id?.toString() === teamIdStr,
    )
    const memberIds = members.map((m) => m.id?.toString() || '')

    // Tenta encontrar uma meta definida diretamente para o Head/Equipe
    const directGoal = goalStore.goals.find(
      (g) =>
    ...
    if (selectedTeamId.value) {
      if (selectedTeamId.value.startsWith('head_own_')) {
        ...
        sellers = sellers.filter(
          (s) => s.id?.toString() === selectedTeamId.value || s.head_id?.toString() === selectedTeamId.value,
        )
      }
    }
  ...
  const allFilteredContracts = computed(() => {
    // Usamos contractsForGeneralPerformance para garantir que o filtro de data (mês selecionado)
    // seja respeitado na lista de contratos também.
    return applyFilters(contractsForGeneralPerformance.value)
  })

  const filteredP1Contracts = computed(() => {
    // Para performance P1, ignoramos cancelados
    return applyFilters(contractsForP1.value).filter((c) => !c.canceled_at)
  })

  const filteredGeneralContracts = computed(() => {
    // Para indicadores gerais de performance, ignoramos cancelados
    return applyFilters(contractsForGeneralPerformance.value).filter((c) => !c.canceled_at)
  })
    ...
    if (user.value.type === 'admin') {
      if (selectedBUId.value) {
        filtered = filtered.filter(
          (s) =>
            (s as any).seller_business?.some((sb: any) => sb.business_id === Number(selectedBUId.value)) ||
            s.id === user.value?.id,
        )
      }
  ...
  const stats = computed(() => {
    ...
    const totalImplementation = genContracts
      .filter((c) => c.signed)
      .reduce((acc, curr) => {
        return acc + (parseFloat(curr.implementation_fee as any) || 0)
      }, 0)

    const totalMonthly = genContracts
      .filter((c) => c.signed)
      .reduce((acc, curr) => {
  ...
  const teamSectionTitle = computed(() => {
    if (selectedTeamId.value && user.value?.type !== 'coord') {
      const teamHead = sellerStore.allSellers.find((s) => s.id?.toString() === selectedTeamId.value)
      return teamHead ? `Equipe de ${teamHead.name}` : 'Gestão de Equipe'
    }
    if (user.value?.type === 'admin') return 'Gestão de Colaboradores'
    if (user.value?.type === 'head') return 'Usuários da Unidade'
    return 'Minha Equipe'
  })
  ...
  const visibleUsers = computed(() => {
    ...
    if (selectedTeamId.value) {
      users = users.filter(
        (u) => u.id?.toString() === selectedTeamId.value || u.head_id?.toString() === selectedTeamId.value,
      )
    }
  ...
  const availableSellers = computed(() => {
    if (user.value?.type !== 'coord') return []
    let available = sellerStore.allSellers.filter(
      (s: Sellers) => (s.type === 'seller' || s.type === 'sdr') && !s.head_id,
    )
  ...
  onMounted(async () => {
    ...
    if (user.value?.id) {
      ...
      if (user.value.type === 'admin') {
        ...
        const myBUIds = (user.value as any).seller_business?.map((sb: any) => sb.business_id) || []
        if (myBUIds.length > 0) {
          // Busca todos os contratos de todas as BUs vinculadas para o Head
  ...
  const handleProfileUpdated = () => {
    ...
    if (user.value?.type === 'coord' && user.value.id) {
  ...
  watch(dashboardFilterType, (newMode) => {
    ...
    if (newMode === 'bu' && buOptionsFormatted.value.length > 0) {
      ...
      // Se for coord, tenta manter ou pegar a primeira equipe real
      if (authStore.user?.type === 'coord') {
