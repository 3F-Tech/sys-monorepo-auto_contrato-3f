...
<template>
  <Transition name="fade">
    <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-brand-deep/80 backdrop-blur-sm">
      <div class="w-full max-w-xl bg-brand-offset/95 border border-brand-glass-border rounded-2xl overflow-hidden shadow-2xl backdrop-blur-md">
        <!-- Header -->
        <div class="p-6 border-b border-brand-glass-border">
          <div class="flex items-center justify-between mb-2">
            ...
            <div :class="['text-xs font-mono uppercase tracking-widest px-2 py-1 rounded bg-brand-surface', statusColor]">
        ...
        <div class="px-6 pb-6 mt-2">
           <div class="bg-brand-deep/50 rounded-xl border border-brand-glass-border p-5 min-h-[160px] font-mono text-sm space-y-2">
              <div v-if="showSuccessMessage" class="flex flex-col items-center justify-center py-4 space-y-3 animate-bounce-subtle">
              ...
              <div v-for="(log, index) in logs" :key="index" class="text-gray-400">
                <span class="text-brand-cyan mr-2">></span> {{ log }}
              </div>
              <div v-if="currentStatus.error" class="text-red-400 font-bold border-l-2 border-red-500 pl-3 py-1 bg-red-500/10">
                <span class="mr-2">X</span> ERROR: {{ currentStatus.error }}
              </div>
              <div v-if="currentStatus.status === 'processing' && !showSuccessMessage" class="animate-pulse text-brand-cyan">
                <span class="mr-2">_</span> aguardando próximo passo...
              </div>
           </div>
        </div>
        ...
        <div class="p-4 bg-brand-surface/20 flex justify-end gap-3">
          ...
          <div v-else class="flex items-center gap-2 text-xs text-gray-500 px-4">
            <svg class="animate-spin h-4 w-4 text-brand-cyan" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
