<template>
  <Teleport to="body">
    <div v-if="isOpen" class="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      <!-- Backdrop -->
      <div class="absolute inset-0 bg-brand-deep/80 backdrop-blur-sm transition-opacity" @click="cancel"></div>
      ...
      >
        <!-- Icon Banner -->
        <div
          class="h-16 w-16 rounded-2xl border-2 flex items-center justify-center mb-2 transition-all duration-500 overflow-hidden"
          :class="[
            isLoading
              ? 'animate-pulse scale-95 opacity-80 border-white/5'
              : type === 'danger'
                ? 'bg-red-500/10 border-red-500/20 text-red-500'
                : type === 'orange'
                  ? 'bg-[#FF4B12]/10 border-[#FF4B12]/20 text-[#FF4B12]'
...
<script setup lang="ts">
  ...
  const props = defineProps({
    ...
    type: {
      type: String,
      default: 'danger',
      validator: (v: string) => ['danger', 'warning', 'info', 'orange'].includes(v),
    },
