  ...
  <Teleport to="body">
    <div v-if="isOpen" class="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      ...
      >
        <!-- Icon Banner -->
        <div 
          class="h-16 w-16 rounded-2xl border-2 flex items-center justify-center mb-2"
          :class="[
            type === 'danger' ? 'bg-red-500/10 border-red-500/20 text-red-500' : 'bg-brand-cyan/10 border-brand-cyan/20 text-brand-cyan'
          ]"
        >
        ...
        <div class="space-y-3">
          ...
          <p class="text-sm font-medium text-white/50 leading-relaxed">{{ message }}</p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 w-full pt-4">
          <button 
            @click="cancel"
            class="px-6 py-4 rounded-xl font-bold text-xs uppercase tracking-widest bg-brand-surface/40 border border-brand-glass-border text-white/60 hover:text-white hover:bg-brand-surface transition-all active:scale-95"
          >
            {{ cancelText }}
          </button>
          ...
          <button 
            @click="confirm"
            class="px-6 py-4 rounded-xl font-bold text-xs uppercase tracking-widest text-[#0a0f1c] shadow-lg transition-all active:scale-95"
            :class="[
              type === 'danger' 
...
const props = defineProps({
  ...
  type: { type: String, default: 'danger', validator: (v: string) => ['danger', 'warning', 'info'].includes(v) },
