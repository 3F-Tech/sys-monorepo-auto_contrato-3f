<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
      <div class="flex flex-wrap items-center gap-3">
        ...
        <div class="min-w-[160px]">
          <label class="text-[9px] font-black text-brand-cyan uppercase tracking-widest mb-1.5 block ml-1">Filtrar por Tipo</label>
    ...
    <div class="flex-1 flex flex-col min-h-0">
      <div class="relative overflow-hidden rounded-3xl border border-white/5 bg-white/[0.03] backdrop-blur-md shadow-2xl flex flex-col min-h-0">
        <div class="overflow-x-auto custom-scrollbar flex-1 min-h-0">
          <table class="w-full text-left border-collapse min-w-[1000px]">
            ...
            <tbody class="divide-y-0">
              <tr v-for="goal in paginatedGoals" :key="goal.id" class="hover:bg-brand-cyan/[0.04] transition-all duration-300 group relative">
                <td class="px-8 py-5">
                  <div class="flex items-center gap-4">
                    <div class="h-10 w-10 rounded-2xl bg-brand-cyan/10 flex items-center justify-center border border-brand-cyan/20 shadow-lg shadow-brand-cyan/5 group-hover:scale-110 transition-transform duration-500">
                      <component :is="goal.target_type === 'bu' ? Building : (goal.target_type === 'head' || goal.target_type === 'team') ? Users : User" 
                        class="h-5 w-5 text-brand-cyan" />
                    </div>
                    <div>
                      <div class="text-xs font-black text-white group-hover:text-brand-cyan transition-colors">{{ getTargetName(goal) }}</div>
                      <div class="text-[9px] text-white/20 font-bold uppercase tracking-widest mt-0.5">ID: {{ goal.target_id }}</div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-5 text-center">
                  <span :class="[
                    ...
                    goal.target_type === 'bu' ? 'bg-blue-500/10 border-blue-500/30 text-blue-400' :
                    goal.target_type === 'team' ? 'bg-purple-500/10 border-purple-500/30 text-purple-400' :
                    goal.target_type === 'head' ? 'bg-brand-cyan/10 border-brand-cyan/30 text-brand-cyan' :
                    'bg-emerald-500/10 border-emerald-500/30 text-emerald-400'
                  ]">
                    {{ goal.target_type === 'bu' ? 'BU' : goal.target_type === 'team' ? 'Equipe' : goal.target_type === 'head' ? 'Head' : 'Vendedor' }}
                  </span>
                </td>
                <td class="px-6 py-5">
                  <div class="flex items-center gap-2">
                    ...
                    <div class="text-[11px] font-black text-white/70 tabular-nums">
                      {{ getMonthName(goal.month) }} <span class="text-white/20 mx-1">/</span> {{ goal.year }}
                    </div>
                  </div>
                </td>
                <td class="px-6 py-5">
              ...
              <tr v-if="filteredGoals.length === 0">
                <td colspan="7" class="px-6 py-32 text-center">
                  <div class="flex flex-col items-center gap-4">
                    <div class="h-16 w-16 rounded-[2rem] bg-brand-cyan/5 flex items-center justify-center animate-pulse">
                      <Target class="h-8 w-8 text-brand-cyan/20" />
                    </div>
                    <div class="space-y-1">
                      <p class="text-[11px] font-black uppercase tracking-[0.4em] text-white/30">Nenhum objetivo para o período</p>
                      <p class="text-[9px] font-bold text-white/10 uppercase tracking-widest">Ajuste os filtros para ampliar a busca</p>
                    </div>
                  </div>
                </td>
              </tr>
        ...
        <!-- Pagination Controls -->
        <div v-if="totalPages > 1" class="px-8 py-4 border-t border-white/5 bg-white/[0.02] flex items-center justify-between">
          <div class="text-[10px] font-black text-white/20 uppercase tracking-widest">
            Página {{ currentPage }} de {{ totalPages }} <span class="mx-2">•</span> {{ filteredGoals.length }} registros
          </div>
          <div class="flex items-center gap-2">
            <button 
...
import { Target, Plus, Edit3, Trash2, Building, Users, User, Calendar, ChevronLeft, ChevronRight, Loader2 } from 'lucide-vue-next';
