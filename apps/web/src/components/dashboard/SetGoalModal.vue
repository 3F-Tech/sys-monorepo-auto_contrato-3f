<template>
  ...
  >
    <!-- Backdrop -->
    <div
      class="absolute inset-0 bg-brand-deep/80 backdrop-blur-md transition-all duration-500"
      @click="close"
    ></div>

    <!-- Modal Content -->
    <div
      class="relative w-full max-w-6xl bg-brand-offset/90 border border-brand-glass-border rounded-[2.5rem] p-6 md:p-10 shadow-3xl overflow-hidden group h-[85vh] flex flex-col backdrop-blur-xl"
    >
      <!-- Decoration -->
      <div
      ...
      <div class="flex-1 overflow-y-auto pr-2 custom-scrollbar relative z-10">
        ...
        <form v-else @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div v-if="!showPeriodsOnly" class="space-y-2">
            <label
              class="text-[10px] font-black text-brand-cyan uppercase tracking-widest ml-1"
              >Tipo de Alvo</label
            >
            <div
              class="relative flex rounded-2xl bg-brand-surface border border-brand-glass-border p-1 gap-0.5"
            >
              <!-- Glider animado -->
              <div
          ...
          <div class="pt-4 border-t border-white/5">
            <div class="flex items-center gap-2 mb-4">
              ...
              <span
                class="text-xs font-bold text-white/70 uppercase tracking-wider"
                >Métricas da Meta (R$)</span
              >
            </div>
            <Transition name="metrics-fade" mode="out-in">
              ...
              >
                <div v-if="!showPeriodsOnly" class="space-y-1.5">
                  ...
                  <div class="relative">
                    <span
                      class="absolute left-4 top-1/2 -translate-y-1/2 text-xs text-white/20"
                      >R$</span
                    >
                    <input
                      v-model="form.p1"
                ...
                <template v-if="showPeriodsOnly">
                  ...
                  <!-- Info Box -->
                  <div
                    class="md:col-span-2 lg:col-span-3 p-4 rounded-2xl bg-white/5 border border-white/10"
                  >
                    <p
                      class="text-[10px] text-white/30 uppercase tracking-wider leading-relaxed"
                    >
                      💡 <strong>Dica:</strong> Os campos acima definem os
                      "marcos" na linha do gráfico. Se deixá-los vazios, o
                      sistema calculará uma distribuição linear baseada na
                      **Meta P1 Total** ({{
                        formatFullCurrency(form.p1 || 0)
                      }}).
                    </p>
                  </div>
                </template>
          ...
          <div class="pt-6 flex gap-3 sticky bottom-0 bg-brand-offset py-2">
            ...
            <button
              type="submit"
              :disabled="loading || !form.target_id"
...
const periodsSum = computed(() => {
  return (
    Number(form.value.p1_period_1 || 0) +
...
const targetIdOptions = computed(() => {
  if (form.value.target_type === "bu") {
    ...
    // Para tipo Seller
    const sellers =
      authStore.user?.type === "head"
        ? sellerStore.teamSellers
  ...
  (newVal) => {
    if (newVal) {
      ...
      if (props.initialView === "form" && props.initialTarget) {
        const cleanId = props.initialTarget.id
          .toString()
          .replace("team_", "")
          .replace("head_own_", "");
        const existing = goalStore.getGoalByTarget(
          props.initialTarget.type,
          cleanId,
        );
        if (existing) {
          editGoal(existing);
...
const editGoal = (goal: Goal) => {
  ...
  form.value = {
    ...
    implementation:
      goal.implementation != null ? Number(goal.implementation) : null,
...
watch(
  ...
  async () => {
    ...
    const existing = goalStore.goals.find(
      (g) =>
    ...
    if (existing) {
      ...
      form.value.p1_period_1 =
        existing.p1_period_1 != null ? Number(existing.p1_period_1) : null;
      form.value.p1_period_2 =
        existing.p1_period_2 != null ? Number(existing.p1_period_2) : null;
      form.value.p1_period_3 =
        existing.p1_period_3 != null ? Number(existing.p1_period_3) : null;
      form.value.p1_period_4 =
        existing.p1_period_4 != null ? Number(existing.p1_period_4) : null;
      ...
      form.value.implementation =
        existing.implementation != null
          ? Number(existing.implementation)
          : null;
      form.value.monthly =
        existing.monthly != null ? Number(existing.monthly) : null;
    } else {
      form.value.p1 = null;
      form.value.p1_period_1 = null;
      form.value.p1_period_2 = null;
