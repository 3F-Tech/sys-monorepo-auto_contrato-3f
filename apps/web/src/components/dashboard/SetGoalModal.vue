        ...
        <form v-else @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="relative flex rounded-2xl bg-brand-surface border border-brand-glass-border p-1 gap-0.5">
              ...
              <button
                ...
                :class="[
                  ...
                  form.target_type === opt.value ? 'text-brand-cyan' : 'text-white/30 hover:text-white/60',
                ]"
              >
          ...
          <div v-if="!showPeriodsOnly" class="grid grid-cols-2 gap-4">
            <div class="space-y-1.5">
              ...
              <CustomSelect v-model="form.month" :options="monthOptions" placeholder="Mês" />
            </div>
            <div class="space-y-1.5">
              <label class="text-[10px] font-semibold text-brand-cyan uppercase tracking-widest">Ano</label>
              <CustomSelect v-model="form.year" :options="yearOptions" placeholder="Ano" />
            </div>
          </div>
                ...
                <template v-if="showPeriodsOnly">
                  ...
                  <div class="md:col-span-2 lg:col-span-3 p-4 rounded-2xl bg-white/5 border border-white/10">
                    <p class="text-[10px] text-white/30 uppercase tracking-wider leading-relaxed">
                      💡
                      <strong>Dica:</strong>
                      Os campos acima definem os "marcos" na linha do gráfico. Se deixá-los vazios, o sistema calculará
                      uma distribuição linear baseada na **Meta P1 Total** ({{ formatFullCurrency(form.p1 || 0) }}).
                    </p>
                  </div>
                </template>
...
<script setup lang="ts">
  ...
  import { Target, X, LoaderCircle, Save, Building2, Users, User, TrendingUp } from '@lucide/vue'
  ...
  watch(
    [() => form.value.target_id, () => form.value.month, () => form.value.year, () => form.value.target_type],
    async () => {
