<template>
  <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-2xl bg-brand-offset border border-brand-glass-border rounded-[2.5rem] shadow-2xl overflow-hidden p-10 space-y-8 animate-in zoom-in-95 duration-300">
      <div class="flex items-center justify-between">
        <div class="space-y-1">
          ...
          <p class="text-xs text-white/40 uppercase tracking-widest font-bold">Defina o custo por aquisição por BU para {{ monthName }} de {{ year }}</p>
        </div>
      ...
      <div class="p-4 rounded-2xl bg-brand-cyan/5 border border-brand-cyan/10 flex items-start gap-4">
        ...
        <p class="text-[10px] text-brand-cyan/80 uppercase font-black tracking-widest leading-relaxed">
          Os valores salvos permanecem os mesmos nos meses seguintes até serem alterados. 
          O valor de <strong>3F Venture</strong> será sempre a média automática das BUs abaixo.
        </p>
      </div>

      <div class="space-y-4 max-h-[400px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-white/10">
        <div v-for="bu in filteredBusinesses" :key="bu.id" 
          ...
          <div class="flex items-center justify-between gap-6">
            <div class="flex items-center gap-4">
              <div class="h-12 w-12 rounded-2xl bg-brand-surface border border-brand-glass-border flex items-center justify-center text-brand-cyan font-black">
                {{ bu.name?.[0] }}
              </div>
              <div class="space-y-0.5">
                ...
                <div v-if="isInherited(bu.id)" class="flex items-center gap-2">
                  ...
                  <p class="text-[8px] text-orange-400/60 uppercase font-black tracking-widest">Valor herdado do mês anterior</p>
                </div>
              </div>
            </div>
      ...
      <div class="flex gap-4 pt-4 border-t border-white/5">
        ...
        <button @click="handleSave" :disabled="saving" class="flex-[2] py-4 rounded-2xl bg-brand-cyan text-brand-deep shadow-xl shadow-brand-cyan/20 text-xs font-black uppercase hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50">
          <span v-if="saving" class="flex items-center justify-center gap-2">
             <Loader2 class="h-4 w-4 animate-spin" /> Salvando...
          </span>
...
import { DollarSign, X, Info, Loader2 } from 'lucide-vue-next';
