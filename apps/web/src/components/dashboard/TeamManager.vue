  ...
  <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
      <div class="space-y-1">
        ...
        <p class="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em]">Crie e gerencie agrupamentos de vendedores</p>
      </div>
    ...
    <!-- Teams Grid -->
    <div v-if="teamStore.loading && teamStore.teams.length === 0" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      <div v-for="i in 3" :key="i" class="h-64 rounded-[2rem] bg-white/[0.02] border border-white/5 animate-pulse"></div>
    </div>

    <div v-else-if="teamStore.teams.length === 0" class="py-20 rounded-[2.5rem] border border-dashed border-white/10 bg-white/[0.01] flex flex-col items-center justify-center text-center space-y-4">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
      <div v-for="team in filteredTeams" :key="team.id"
        ...
        <!-- Abstract Background -->
        <div class="absolute -top-24 -right-24 w-48 h-48 bg-brand-cyan/5 blur-3xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>

        <div class="flex items-start justify-between relative z-10">
          <div class="flex items-center gap-4">
            <div class="h-16 w-16 rounded-2xl bg-brand-surface border border-brand-glass-border flex items-center justify-center shrink-0 overflow-hidden">
            ...
            <div>
              ...
              <p class="text-[10px] font-bold text-brand-cyan/60 uppercase tracking-widest">{{ team.sellers_sellers_team_idToteams?.length || 0 }} Membros</p>
            </div>
          </div>
          
          <div class="flex items-center gap-2">
            <button @click="handleEditTeam(team)" class="p-2 rounded-xl bg-white/5 border border-white/5 text-white/40 hover:text-brand-cyan hover:bg-brand-cyan/10 hover:border-brand-cyan/20 transition-all">
              <Edit2 class="h-4 w-4" />
            </button>
            <button @click="handleDeleteTeam(team.id)" class="p-2 rounded-xl bg-white/5 border border-white/5 text-white/40 hover:text-red-400 hover:bg-red-400/10 hover:border-red-400/20 transition-all">
              <Trash2 class="h-4 w-4" />
            </button>
          </div>
        </div>

        <p class="text-xs text-white/40 leading-relaxed min-h-[3em] line-clamp-2 relative z-10">{{ team.description || 'Sem descrição definida.' }}</p>
        ...
        <div class="space-y-4 pt-6 border-t border-white/5 relative z-10">
          <div class="flex items-center justify-between">
             ...
             <button @click="prepareAddMember(team)" class="text-[9px] font-black text-brand-cyan hover:text-white uppercase tracking-widest transition-colors flex items-center gap-1">
          ...
          <div class="flex flex-wrap gap-2">
            <div v-for="member in team.sellers_sellers_team_idToteams" :key="member.id"
              ...
              <div class="h-5 w-5 rounded-full bg-brand-cyan/20 flex items-center justify-center text-[8px] font-black text-brand-cyan">
                {{ member.name?.[0] }}
              </div>
              <span class="text-[9px] font-bold text-white/60 group-hover/member:text-white">{{ member.name }}</span>
              <button @click="handleRemoveMember(team.id, member.id)" class="opacity-0 group-hover/member:opacity-100 transition-opacity hover:text-red-400 p-0.5">
                <X class="h-2.5 w-2.5" />
              </button>
            </div>
            <div v-if="!team.sellers_sellers_team_idToteams?.length" class="text-[9px] font-medium text-white/10 italic">Nenhum vendedor associado.</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showCreateModal || editingTeam" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-xl bg-brand-offset border border-brand-glass-border rounded-[2.5rem] shadow-2xl overflow-hidden p-10 space-y-8 animate-in zoom-in-95 duration-300">
        <div class="space-y-2">
          <h3 class="text-2xl font-black text-white tracking-tighter uppercase">{{ editingTeam ? 'Editar Equipe' : 'Nova Equipe' }}</h3>
          <p class="text-xs text-white/40 uppercase tracking-widest font-bold">Defina os detalhes fundamentais da equipe</p>
        </div>
        ...
        <div class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-1 space-y-2">
              <label class="text-[10px] font-black text-brand-cyan/60 uppercase tracking-[0.2em] ml-1">Foto da Equipe</label>
              <div 
                ...
                :class="[
                  ...
                  form.photo_url ? 'border-solid border-brand-cyan/30' : 'hover:border-brand-cyan/50 hover:bg-brand-cyan/5'
                ]"
              >
                <img v-if="form.photo_url" :src="form.photo_url" class="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover/upload:scale-110" />
                
                <div v-if="!form.photo_url" class="relative z-10 flex flex-col items-center gap-2 text-center p-4">
                  <div class="p-3 rounded-2xl bg-brand-cyan/10 text-brand-cyan group-hover/upload:scale-110 group-hover/upload:bg-brand-cyan group-hover/upload:text-brand-deep transition-all duration-500">
                    <Plus class="h-6 w-6" />
                  </div>
                  <p class="text-[8px] font-black text-white/20 uppercase tracking-widest leading-tight">Solte a imagem ou clique</p>
                </div>

                <!-- Overlay on hover if has image -->
                <div v-else class="absolute inset-0 bg-brand-deep/60 backdrop-blur-sm opacity-0 group-hover/upload:opacity-100 transition-opacity flex flex-col items-center justify-center text-white gap-2">
                   <Edit2 class="h-5 w-5" />
            ...
            <div class="md:col-span-2 space-y-6">
              <div class="space-y-2">
                <label class="text-[10px] font-black text-brand-cyan/60 uppercase tracking-[0.2em] ml-1">Nome da Equipe</label>
                <input v-model="form.name" type="text" placeholder="Ex: Guerreiros do Growth" class="w-full px-6 py-4 rounded-2xl bg-brand-surface border border-brand-glass-border text-sm text-white focus:outline-none focus:border-brand-cyan/50 focus:bg-brand-cyan/5 transition-all" />
              </div>
              <div class="space-y-2">
        ...
        <div class="flex gap-4">
          <button @click="closeFormModal" class="flex-1 py-4 rounded-2xl bg-white/5 border border-white/10 text-xs font-black uppercase text-white/40 hover:bg-white/10 hover:text-white transition-all">Cancelar</button>
          <button @click="saveTeam" :disabled="!form.name || teamStore.loading" class="flex-1 py-4 rounded-2xl bg-brand-cyan text-brand-deep shadow-xl shadow-brand-cyan/20 text-xs font-black uppercase hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50">
    ...
    <div v-if="selectingTeamForMember" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-lg bg-brand-offset border border-brand-glass-border rounded-[2.5rem] shadow-2xl overflow-hidden p-10 space-y-8 animate-in zoom-in-95 duration-300">
        <div class="flex items-center justify-between">
          <div class="space-y-1">
            <h3 class="text-xl font-black text-white tracking-tighter uppercase">Adicionar Membros</h3>
            <p class="text-[10px] text-brand-cyan uppercase tracking-widest font-bold">{{ selectingTeamForMember.name }}</p>
          </div>
          <button @click="selectingTeamForMember = null" class="p-2 text-white/20 hover:text-white transition-colors"><X class="h-5 w-5" /></button>
        </div>
        ...
        <div class="max-h-[400px] overflow-y-auto space-y-3 pr-2 scrollbar-thin scrollbar-thumb-white/10">
          <div v-for="seller in availableSellers" :key="seller.id"
            ...
            <!-- Background detail -->
            <div class="absolute -right-4 -top-4 w-12 h-12 bg-brand-cyan/5 blur-xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
            
            <div class="flex items-center gap-4 relative z-10">
               <div class="h-12 w-12 rounded-2xl bg-gradient-to-br from-brand-cyan/20 to-brand-blue/20 border border-brand-cyan/20 flex items-center justify-center text-brand-cyan font-black text-lg group-hover:scale-110 transition-transform duration-500">
                 {{ seller.name?.[0] }}
               </div>
               <div>
                 <p class="text-sm font-bold text-white group-hover:text-brand-cyan transition-colors">{{ seller.name }}</p>
                 <div class="flex items-center gap-2 mt-0.5">
                   <div class="h-1 w-1 rounded-full bg-brand-cyan/40"></div>
                   <p class="text-[8px] text-white/40 uppercase font-black tracking-widest group-hover:text-white/60 transition-colors">Disponível para equipe</p>
                 </div>
               </div>
            </div>
          ...
          <div v-if="availableSellers.length === 0" class="text-center py-10 space-y-2">
            ...
            <p class="text-[9px] text-white/10 italic leading-relaxed uppercase">Todos os seus vendedores já estão em equipes ou ainda não foram criados.</p>
          </div>
        </div>
      </div>
    </div>
...
import { Users2, Plus, Edit2, Trash2, X, Search, Settings2, Check } from 'lucide-vue-next';
...
const filteredTeams = computed(() => {
  ...
  // Se for Head de BU (Coord) ou Coordenador (Head de Equipe)
