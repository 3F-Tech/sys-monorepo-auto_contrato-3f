      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div class="relative z-10 flex flex-col items-center justify-center h-full space-y-4">
            <div class="flex flex-col items-center gap-4 w-full">
              ...
              <div class="flex p-1 bg-white/5 rounded-xl border border-white/5 w-fit">
                <button 
                  @click="radialTab = 'p1'" 
                  :class="radialTab === 'p1' ? 'bg-brand-cyan text-brand-deep shadow-lg scale-105' : 'text-white/40 hover:text-white'" 
                  class="px-5 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all duration-300"
                >P1</button>
                <button 
                  @click="radialTab = 'tcv'" 
                  :class="radialTab === 'tcv' ? 'bg-brand-cyan text-brand-deep shadow-lg scale-105' : 'text-white/40 hover:text-white'" 
                  class="px-5 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all duration-300"
                >TCV</button>
              </div>
            </div>
            ...
            <div class="relative w-full aspect-square max-w-[240px]">
              ...
              <div class="absolute inset-0 flex flex-col items-center justify-center -translate-y-2">
                <span class="text-4xl font-black text-white tracking-tighter">
                  {{ radialMetric.percent.toFixed(0) }}<span class="text-brand-cyan text-xl">%</span>
                </span>
                <span class="text-[9px] font-bold text-white/20 uppercase tracking-widest mt-1">{{ radialMetric.key.toUpperCase() }} Atingido</span>
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-8 w-full pt-4 border-t border-brand-cyan/10">
              <div class="text-center">
                ...
                <p class="text-sm font-bold text-brand-cyan">{{ formatCurrency(radialMetric.actual) }}</p>
              </div>
              <div class="text-center border-l border-brand-cyan/10">
                ...
                <p class="text-sm font-bold text-white/60">{{ formatCurrency(radialMetric.target) }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="lg:col-span-8 p-8 rounded-[2.5rem] bg-brand-cyan/[0.03] border border-brand-cyan/10 relative overflow-hidden">
          <div class="flex items-center justify-between mb-8">
            <span class="text-[10px] font-black text-brand-cyan/40 uppercase tracking-[0.2em]">Evolução P1 (Meta vs Realizado)</span>
          ...
          <div class="space-y-4 relative z-10">
            ...
            <div class="space-y-2">
              ...
              <div class="flex items-center justify-between">
                <span class="text-[9px] font-medium text-white/50 uppercase">Realizado: {{ formatCurrency(metric.actual) }}</span>
...
watch(() => props.goal, () => lineChartKey.value++, { deep: true });
...
const radialMetric = computed(() => {
  return displayMetrics.value.find(m => m.key === radialTab.value) || { 
    key: radialTab.value, label: '', target: 0, actual: 0, percent: 0, 
    remaining: 0, barColor: '', percentColor: '' 
  };
});
...
const lineOptions = computed(() => ({
  chart: {
    ...
    dropShadow: { enabled: true, top: 10, left: 0, blur: 15, opacity: 0.2, color: '#00d4ff' }
  },
  ...
  markers: { size: 4, strokeColors: '#00d4ff', strokeWidth: 2, colors: ['#0b0f1a'] },
  tooltip: {
