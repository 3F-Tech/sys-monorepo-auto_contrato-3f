  ...
  <div class="space-y-4">
    ...
    <div v-if="loading" class="space-y-3">
      <div v-for="i in 3" :key="i"
        class="p-1 px-2 rounded-2xl bg-brand-surface/30 border border-brand-glass-border animate-pulse flex items-center justify-between relative overflow-hidden">
...
const backgroundSyncPending = async (contractsToSync: Contracts[]) => {
  ...
  for (const contract of limited) {
    ...
    try {
      ...
      // Qualquer outro erro (400, 500, rede) é silencioso — é uma operação de background não-crítica
    }
  }
}
