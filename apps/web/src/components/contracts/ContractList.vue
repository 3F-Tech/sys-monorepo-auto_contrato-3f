  ...
  <div class="space-y-4">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-1 p-1 bg-black/20 border border-brand-glass-border rounded-xl">
          <button
            v-for="opt in signedFilterOptions"
            :key="opt.value"
      ...
      <div class="relative group w-full md:w-80">
        ...
        <input
          v-model="searchQuery"
          type="text"
          placeholder="Buscar contratante ou plano..."
    ...
    <div v-else class="space-y-3">
      ...
      >
        <!-- Header / Summary -->
        <div
          @click="toggleExpand(contract.id?.toString()!)"
        ...
        >
          <div class="flex items-center gap-4">
            ...
            >
              <img
                v-if="getBuImage(contract.bu_id)"
                :src="getBuImage(contract.bu_id)!"
                class="h-full w-full object-cover"
                :alt="getBuName(contract.bu_id)"
              />
          ...
          <div class="flex items-center gap-6">
            ...
            <div class="hidden lg:flex flex-col items-end gap-1">
              ...
              >
                <div
                  ...
                  :class="getSignaturesStatus(contract).dot"
                ></div>
                <span
            ...
            <div class="hidden md:flex flex-col items-end gap-1">
              ...
              <span
                v-if="contract.change_status === 'alert'"
              ...
              <span
                v-else-if="contract.change_status === 'approved'"
              ...
              <span
                v-else-if="contract.change_status === 'reject'"
        ...
        <Transition name="expand">
          <div
            v-if="expandedId === contract.id?.toString()"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
              ...
              <div class="space-y-6">
                <div class="space-y-3">
                  ...
                  <div class="flex gap-2">
                    ...
                    <button
                      @click="handleUpdateLink(contract)"
                ...
                <div class="p-5 rounded-2xl bg-black/40 border border-brand-glass-border shadow-md space-y-4">
                  ...
                  <div class="pt-4 border-t border-white/5 space-y-4">
                    <div class="flex items-center justify-between">
                      ...
                      <button
                        v-if="isEditingDates !== contract.id?.toString()"
                        @click="startEditingDates(contract)"
              ...
              <div class="space-y-6">
                <div class="flex items-center justify-between">
                  ...
                  <button
                    v-if="(contract as any).canceled_at || !(contract as any).approved"
                ...
                <div class="grid grid-cols-2 gap-4">
                  <button
                    v-if="(contract as any).approved && !(contract as any).canceled_at"
                  ...
                  <button
                    ...
                    "
                    @click="openChangeModal(contract)"
                    :disabled="!contract.link || contract.signed"
                    :class="[
                  ...
                  <button
                    v-if="!(contract as any).approved && !(contract as any).canceled_at"
                    @click="confirmSendToClicksign(contract)"
                    :disabled="!contract.link || isSendingToClicksign"
                    :class="[
                      'p-6 rounded-2xl border-2 flex flex-col items-center justify-center gap-3 transition-all duration-300 shadow-lg group/btn',
                  ...
                  <button
                    v-if="(contract as any).approved && !(contract as any).canceled_at"
                    @click="confirmCancelContract(contract)"
                  ...
                  <button
                    v-if="(contract as any).canceled_at"
                    disabled
                  ...
                  <div
                    v-else-if="!isLeadership && contract.change_status === 'alert'"
                  ...
                  >
                    <button
                      @click="cancelChangeRequest(contract)"
                    ...
                    <p
                      v-if="contract.change_description"
                  ...
                  <div
                    v-if="isLeadership && contract.change_status === 'alert'"
                ...
                <div
                  v-if="contract.signed || (contract as any).canceled_at"
                ...
                >
                  <component
                    :is="contract.signed ? CheckCircle2 : Ban"
                    :class="['h-3 w-3', contract.signed ? 'text-green-400/50' : 'text-red-400/50']"
                  />
                  <p
                    class="text-[9px] font-bold uppercase tracking-widest text-center"
        ...
        >
          ...
          <div class="space-y-3">
            ...
            <p class="text-xs text-white/50 mb-2 leading-relaxed">
              Nossa equipe de liderança irá analisar o motivo descrito antes de aprovar ou recusar o aviso de mudança.
              Seja detalhista.
            </p>
            <textarea
              ...
              class="w-full px-5 py-4 rounded-2xl bg-brand-offset border border-brand-glass-border text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-orange-500/50 transition-all resize-none shadow-inner"
            ></textarea>
          </div>

          <div class="flex items-center justify-end gap-3 pt-6 border-t border-brand-glass-border">
            <button
              @click="changeRequestModalOpen = false"
              class="px-6 py-3 rounded-xl font-bold text-sm text-white/40 hover:text-white hover:bg-white/5 transition-all"
            >
              Cancelar
            </button>
            <button
      ...
      <div v-if="reviewModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          <div class="flex items-center gap-4">
            ...
            >
              <img
                v-if="getBuImage(reviewContract?.bu_id)"
                :src="getBuImage(reviewContract?.bu_id)!"
                class="h-full w-full object-cover"
              />
      ...
      <div v-if="showCancelContractModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <p class="text-white/70 mb-6 leading-relaxed">
            ...
            plataforma
            <strong class="text-white">Clicksign</strong>
            , e os signatários não poderão mais assiná-lo.
          </p>

          <div class="flex justify-end gap-3 mt-auto">
            <button
              @click="showCancelContractModal = false"
              class="px-5 py-2.5 rounded-xl border border-white/10 text-white/70 hover:bg-white/5 transition-colors font-medium text-sm"
            >
              Voltar
            </button>
            <button
              @click="cancelContractStatus"
            ...
            >
              <span
                v-if="isCanceling"
                class="h-3 w-3 border-2 border-white/20 border-t-white rounded-full animate-spin"
              ></span>
    ...
    <ConfirmModal
      :isOpen="showSendToClicksignModal"
      title="Enviar para Clicksign"
      ...
      type="orange"
      :isLoading="isSendingToClicksign"
      :loadingMessage="progressMessage"
      @confirm="executeSendToClicksign"
      @cancel="showSendToClicksignModal = false"
    >
      <template #icon>
...
<script setup lang="ts">
  ...
  const backgroundSyncPending = async (contractsToSync: Contracts[]) => {
    ...
    const candidates = contractsToSync.filter(
      (c) => !c.signed && !c.canceled_at && ((c as any).envelope_id || c.document_id),
    )
  ...
  watch(
    [() => props.contracts, signedFilter],
    ([newContracts, newFilter]) => {
  ...
  const filteredContracts = computed(() => {
    ...
    if (searchQuery.value) {
      ...
      displayContracts = displayContracts.filter(
        (c) =>
  ...
  const calculateDuration = (contract: Contracts) => {
    ...
    const end = contract.signed && contract.signed_date ? new Date(contract.signed_date as string) : new Date()
