    ...
    <div class="space-y-6">
      <div class="flex items-center gap-3 border-b border-brand-glass-border pb-2">
        ...
        <h3 class="text-sm font-black uppercase tracking-[0.2em] text-white/90">
          Signatários da {{ buName }} (Contratada/Testemunhas)
        </h3>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="space-y-2 opacity-60">
          ...
          <input
            type="text"
            :value="fixedSignatories.contracted.name"
            class="input-glass w-full cursor-not-allowed"
            readonly
          />
        </div>
        <div class="space-y-2 opacity-60">
          ...
          <input
            type="text"
            :value="fixedSignatories.contracted.email"
            class="input-glass w-full cursor-not-allowed"
            readonly
          />
        </div>
        <div class="space-y-2 opacity-60">
          ...
          <input
            type="text"
            :value="fixedSignatories.contracted.cpf"
            class="input-glass w-full cursor-not-allowed"
            readonly
          />
        </div>
        ...
        <div class="pt-6 border-t border-white/5 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-2 opacity-60 text-brand-cyan">
            <label class="text-[10px] font-bold text-white/40 uppercase tracking-widest">
              TESTEMUNHA {{ idx + 1 }} (FIXA)
            </label>
            <input
              type="text"
              :value="witness.name"
              class="input-glass w-full cursor-not-allowed !border-brand-cyan/20"
              readonly
            />
          </div>
          <div class="space-y-2 opacity-60">
            <label class="text-[10px] font-bold text-white/40 uppercase tracking-widest">
              EMAIL TESTEMUNHA {{ idx + 1 }}
            </label>
            <input type="text" :value="witness.email" class="input-glass w-full cursor-not-allowed" readonly />
          </div>
          <div class="space-y-2 opacity-60">
            <label class="text-[10px] font-bold text-white/40 uppercase tracking-widest">
              CPF TESTEMUNHA {{ idx + 1 }}
            </label>
      ...
      <div class="pt-6 border-t border-white/5 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="space-y-2 opacity-60">
          <label class="text-[10px] font-bold text-white/40 uppercase tracking-widest">
            TESTEMUNHA {{ fixedSignatories.witnesses.length + 1 }} (VENDEDOR)
          </label>
          <input
            type="text"
            id="NOME VENDEDOR"
            :value="form['NOME VENDEDOR']"
            class="input-glass w-full cursor-not-allowed"
            readonly
          />
        </div>
        <div class="space-y-2 opacity-60">
          <label class="text-[10px] font-bold text-white/40 uppercase tracking-widest">
            EMAIL TESTEMUNHA {{ fixedSignatories.witnesses.length + 1 }}
          </label>
          <input
            type="text"
            id="EMAIL VENDEDOR"
            :value="form['EMAIL VENDEDOR']"
            class="input-glass w-full cursor-not-allowed"
            readonly
          />
        </div>
        <div class="space-y-2 opacity-60">
          <label class="text-[10px] font-bold text-white/40 uppercase tracking-widest">
            CPF TESTEMUNHA {{ fixedSignatories.witnesses.length + 1 }}
          </label>
          <input
            type="text"
            id="CPF VENDEDOR"
            :value="form['CPF VENDEDOR']"
            class="input-glass w-full cursor-not-allowed"
            readonly
          />
        </div>
      ...
      <!-- Bloco: Coordenador da BU (Select) -->
      <div class="pt-6 border-t border-white/5">
        <div class="flex items-center gap-3 border-b border-brand-glass-border pb-2 mb-4">
          ...
          <h3 class="text-sm font-black uppercase tracking-[0.2em] text-white/90">
            TESTEMUNHA {{ fixedSignatories.witnesses.length + 2 }} (COORDENADOR DA BU)
          </h3>
        </div>

        <div v-if="loadingCoords" class="text-white/30 text-xs italic py-2">Buscando coordenadores...</div>
        <div
          v-else-if="coordOptions.length === 0"
          class="p-3 rounded-xl bg-white/5 border border-brand-glass-border text-white/30 text-xs italic"
        >
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-2 md:col-span-2">
            ...
            <CustomSelect
              ...
              placeholder="SELECIONE O COORDENADOR DA BU"
              searchable
              allow-clear
              variant="form"
              class="w-full"
      ...
      >
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="space-y-2">
            ...
            <input
              type="text"
              :id="'NOME TESTEMUNHA ' + i"
              v-model="form['NOME TESTEMUNHA ' + i]"
            ...
            />
            <span
              v-if="errors && errors['NOME TESTEMUNHA ' + i]"
              role="alert"
          ...
          <div class="space-y-2">
            ...
            <input
              type="email"
              :id="'EMAIL TESTEMUNHA ' + i"
              v-model="form['EMAIL TESTEMUNHA ' + i]"
              class="input-glass w-full"
            ...
            />
            <span
              v-if="errors && errors['EMAIL TESTEMUNHA ' + i]"
              role="alert"
              class="text-[9px] text-red-500 font-bold mt-1 block"
            >
          ...
          <div class="space-y-2">
            ...
            />
            <span
              v-if="errors && errors['CPF TESTEMUNHA ' + i]"
              role="alert"
              class="text-[9px] text-red-500 font-bold mt-1 block"
            >
...
<script setup lang="ts">
  ...
  import { User as UserIcon, Plus, Trash2, ShieldCheck } from '@lucide/vue'
  ...
  const coordOptions = computed(() => {
    ...
    console.log('allCoords: ', allCoords.value)
