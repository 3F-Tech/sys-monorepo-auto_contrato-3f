<template>
  <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <!-- Modal Content -->
    <div class="relative w-full max-w-md bg-brand-offset border border-brand-glass-border rounded-3xl p-8 shadow-2xl overflow-hidden group">
      <!-- Decoration -->
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div class="flex flex-col items-center justify-center space-y-3 pb-4 border-b border-white/5">
            <div class="relative group/logo">
              <div :style="{ borderColor: form.color }" 
                ...
                <Building2 v-else class="h-10 w-10 text-white/10" :style="{ color: !form.img_base64 ? form.color + '20' : '' }" />
                
                <div class="absolute inset-0 bg-black/60 opacity-0 group-hover/logo:opacity-100 transition-opacity flex items-center justify-center cursor-pointer" @click="triggerFileUpload">
                  <Camera class="h-6 w-6 text-white" />
                </div>
              </div>
              <button type="button" @click="triggerFileUpload" class="absolute -bottom-2 -right-2 p-2 rounded-xl bg-brand-cyan text-brand-deep shadow-lg hover:scale-105 active:scale-95 transition-all">
          ...
          <div class="space-y-1.5">
            ...
            <input v-model="form.cnpj" type="text" v-maska="'##.###.###/####-##'" required
              class="w-full bg-brand-surface border border-brand-glass-border rounded-xl px-4 py-3 text-sm text-white focus:border-brand-cyan/40 focus:outline-none transition-all"
...
import { PlusCircle, Edit3, X, Loader2, Camera, Upload, Building2 } from 'lucide-vue-next';
