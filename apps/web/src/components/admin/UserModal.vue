  ...
  >
    ...
    >
      ...
      >
        ...
        <!-- Seção: Hierarquia -->
        <div
          v-if="form.type === 'seller' || form.type === 'sdr'"
          class="space-y-1.5"
        >
          <label
            class="text-[10px] font-semibold text-brand-cyan uppercase tracking-widest"
        ...
        <div class="space-y-3">
          ...
          >
            ...
            {{
              form.type === "coord"
                ? "Unidade de Negócio Coordenada"
                : "Unidades de Negócio Vinculadas"
            }}
          </label>

          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            <label
              v-for="bu in allBusiness"
              :key="bu.id"
            ...
            >
              ...
              >
                <Building2
                  ...
                  :style="{ color: bu.color || undefined }"
                />
              </div>
...
const loadInitialData = async () => {
  try {
    ...
    const fetchedHeads = (sellersData as Sellers[]).filter(
      (s) => s.type === "head",
    );
    ...
    if (authStore.userRole === "head" && authStore.user) {
      const alreadyInList = fetchedHeads.some(
        (h) => h.id?.toString() === authStore.user?.id?.toString(),
      );
      if (!alreadyInList) {
...
watch(
  ...
  (newType) => {
    if (newType === "coord" && selectedBUs.value.length > 1) {
      ...
      toast.warning(
        "Heads de BU podem estar vinculados a apenas uma unidade. Mantendo a última selecionada.",
      );
    }
  },
);
  ...
  async (newVal) => {
    if (newVal) {
      ...
      if (props.user) {
        ...
        selectedBUs.value =
          props.user.seller_business?.map((sb: any) => sb.business_id) || [];
      } else {
        form.value = {
          name: "",
          email: "",
          ...
          head_id:
            authStore.userRole === "head" &&
            availableRoles.value[0]?.value === "seller"
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const sellerPayload: any = {
      ...
      head_id:
        form.value.head_id && form.value.head_id !== "null"
