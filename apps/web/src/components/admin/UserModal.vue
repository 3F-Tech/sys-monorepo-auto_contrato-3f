      ...
      <form @submit.prevent="handleSubmit" class="relative z-10 space-y-6 overflow-y-auto pr-2 custom-scrollbar">
        ...
        <div v-if="form.type === 'seller' || form.type === 'sdr'" class="space-y-1.5">
          ...
          <CustomSelect
            ...
            :disabled="authStore.userRole === 'coord'"
            searchable
            allow-clear
          />
        </div>
...
<script setup lang="ts">
  ...
  import { UserPlus, UserCog, X, Loader2, Building2, Save } from '@lucide/vue'
      ...
      if (newVal) {
        ...
        if (props.user) {
          ...
          form.value = {
            ...
            head_id:
              authStore.userRole === 'coord' && availableRoles.value[0]?.value === 'seller'
