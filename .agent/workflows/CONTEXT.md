...
- **V1 (atual):** CRUDs de usuários e BUs, autenticação JWT própria, **Integração Clicksign (Fase 1: Envio de Documentos)**, dashboard de métricas básicas com filtros por mês/BU/vendedor, fluxo de aprovação de alterações de contrato (`change_status`).
